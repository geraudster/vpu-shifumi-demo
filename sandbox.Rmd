...
sess = k_get_session()
                                        ...
                                        # rename output node
tf$identity(model$output[1], name='output0')
model$output_names

optimized_graph <- sess$graph$as_graph_def()
optimized_graph <- tf$graph_util$convert_variables_to_constants(sess, optimized_graph, list('output0'))
...
```{sh}
docker run -v /home/geraud/projects/keras_transfer_learning/frozen/alpha_retrained.pb:/model.pb -it geraudster/ncs-container mvNCCompile /model.pb -in 'input_1' -on 'output0'
```
