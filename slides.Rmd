...
img_preproc <- function(img) {
    image_load(img) %>%
        image_to_array %>%
        array_reshape(c(1, img_width, img_height, 3))
}
...
```{python,device_open,eval=py$device_list}
...
```{python,device_options,eval=py$device_list}
...
```{python,graph_to_buffer,eval=py$device_list}
...
```{python,init_graph,eval=py$device_list}
...
```{python,init_fifo,eval=py$device_list}
...
```{python,first_inference,eval=py$device_list}
import cv2
import numpy
...
```{python,np_zeros,echo=FALSE}
import numpy as np
tensor = np.zeros([128,128,3], numpy.float32)
```

L'image est ensuite poussée dans la file d'entrée :
```{python,queue_inference,eval=py$device_list}
...
```{python,read_inference,eval=py$device_list}
...
```{python,clean_all_1,eval=py$device_list}
input_fifo.destroy()
output_fifo.destroy()
graph.destroy()
device.close()
...
```{python,benchmark_ncs,echo=FALSE,eval=py$device_list}
from mvnc import mvncapi
...
```{python,clean_all_2,echo=FALSE,eval=py$device_list}
input_fifo.destroy()
output_fifo.destroy()
graph.destroy()
device.close()
...
```
VPU :
```{r,summary_vpu,echo=FALSE,eval=py$device_list}
...
```{r,train_model_transfer,echo=FALSE}
model %>% compile(optimizer = 'rmsprop',
                  loss = 'categorical_crossentropy',
                  metrics = c('accuracy'))
history <- model %>% fit_generator(train_generator,
...
## {data-background=slides/422.jpeg data-background-size=contain}
