...
```{r,MLP_training_with_dataset,out.width='450px', fig.align='center', echo=FALSE}
...
```{python,first_inference,eval=py$has_device}
...
```{python,np_zeros,echo=FALSE,eval=py$has_device}
import numpy as np
...
##base_model <- application_densenet121(weight = 'imagenet',
##                                   include_top = FALSE,
##                                   input_shape = c(img_width, img_height, 3),
##                                   pooling = 'avg')
##

##base_model <- application_inception_resnet_v2(weight = 'imagenet',
##                                              include_top = FALSE,
##                                              input_shape = c(img_width, img_height, 3))
##
...
```{bash,ncscompile_model_transfer,echo=FALSE}
PYTHONPATH="${PYTHONPATH}:/opt/movidius/caffe/python"
source /opt/movidius/virtualenv-python/bin/activate
mkdir -p $PWD/target/ncs
